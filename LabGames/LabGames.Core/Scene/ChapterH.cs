...
namespace LabGames.Core.Scene
{
    ...
    internal class ChapterH : Chapter
    {
        ...
        public override List<string> ExecuteSpecialEvents(Player player, TimeManager time)
        {
            ...
            if (player.hasJob)
            {
                if (time.CurrentStep.dayOfWeek == DayOfWeek.Friday 
                    && time.CurrentStep.partOfDay == PartOfDay.Evening)
                {
                    Random r = new Random();
                    int money = (1200 + r.Next(-110, 100));
                    string genderword = player.Gender == GenderType.Man ? "виконував" : "виконувала";
                    if (player.AssignedWork <= player.WorkTiles)
                    {
                        result.Add($"{player.Name} гарно {genderword} роботу " +
                              $"впродовж тижня і отримує зарплатню: {money} грн.");
                        player.ChangeMoney(money);
                        result.Add($"І це не може не радувати! {Resource.MINUS_HAPPY}");
                        player.ChangeHappines(5);
                    }
                    else
                    {
                        result.Add($"{player.Name} погано {genderword} роботу " +
                              $"впродовж тижня і НЕ ОТРИМУЄ зарплатню на цьому тижні.");
                    }
                    player.AssignedWork = 15;
                    player.WorkTiles = 0;
                }
            }
